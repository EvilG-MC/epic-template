    ...
    public async loadMenus() {
        ...
        await Promise.all(
            files.map(async (file) => {
                ...
                const selectTypes = [
                    ActionRowType.StringMenu,
                    ActionRowType.UserMenu,
                    ActionRowType.RoleMenu,
                    ActionRowType.ChannelMenu,
                    ActionRowType.SelectMenu
                ];
    
                if (!selectTypes.includes(menu.type)) return table.addRow(menu.customId, "Invalid menu type.");
